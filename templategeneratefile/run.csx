...
{
    log.Info("Starting creation of file from template...");
    ...
    const string DEFAULT_template_CODE = "./defaults/default.html";
    ...
    try
    {
        ...
        if (lang == null)
        {
            ...
            {
                string vv = System.Environment.GetEnvironmentVariable("TEMPLATEFILEGEN_DEFAULT_lang");
                if (vv != null)
                {
                    lang = vv;
        ...
        ParamInfo template_transf = handleParameter(json, config_json, "template", true, rootloc, DEFAULT_template_CODE, lang, log);
        AddResponseParam(fileInfo, template_transf, false, false);
        ...
        // Now merge search_replace_data the defaults in configuration and the ones from the request (request overrides configuration)
        JObject search_replace_dataobj_req = new JObject();
        // Take the search_replace_data from the request (if exists) 
        if (json != null && json.search_replace_data != null)
        {
            search_replace_dataobj_req = json.search_replace_data;
        }
        JObject search_replace_dataobj = new JObject();
        // Take default data from configuration (if exists)
        if (config_json != null && config_json.search_replace_data != null)
        {
            search_replace_dataobj = config_json.search_replace_data;
        }

        // merge objects - request overrides default configuration data
        search_replace_dataobj.Merge(search_replace_dataobj_req);
        // Add to fileInfo
        fileInfo.Add("search_replace_data", search_replace_dataobj);

        // Dictionary values of JSON's data
        IDictionary<string, string> data_dic = getRootProperties(dataobj);

        // Dictionary values of JSON's search_replace_data
        IDictionary<string, string> search_replace_data_dic = getRootProperties(search_replace_dataobj);

        using (WebClient wclient = new WebClient())
        {
            fileByteArray = wclient.DownloadData(template_transf.value);
        }

        fileByteArray = doFileGen(fileByteArray,data_dic,search_replace_data_dic);
...
private static byte[] doFileGen(byte[] fileByteArray,IDictionary<string,string> data_dic, IDictionary<string, string> search_replace_data_dic)
{
    string filestr = Encoding.UTF8.GetString(fileByteArray, 0, fileByteArray.Length);
    foreach (KeyValuePair<string, string> prop in search_replace_data_dic)
    {
        filestr = filestr.Replace(prop.Key, prop.Value);
    }
    foreach (KeyValuePair<string, string> prop in data_dic)
    {
        filestr = filestr.Replace("%" + prop.Key + "%", prop.Value);
    }
    fileByteArray = Encoding.UTF8.GetBytes(filestr);
...
private static ParamInfo handleParameter(JObject json, JObject config_json, string name, Boolean isSetting, string rootloc, string defaultVal, string lang, TraceWriter log)
{
    ...
    if (!paramResolved)
    {
        string config_DEFAULT_SettingOrConnectionStringName = "TEMPLATEFILEGEN_DEFAULT_" + name;
        string v = null;
        if (isSetting)
        {
...
private static byte[] setWatermark(byte[] source, string watermark, TraceWriter log)
{
    ...
    if (filestr.Contains("<html") && filestr.Contains("</html>") && filestr.Contains("<body"))
    {
        ...
        if (ibhs > 0 && ibhe > ibhs)
        {
            filestr = filestr.Substring(0, ihtmlte+1) + "<title>TEST ENVIRONMENT INFORMATION: "+watermark+"</title>" + filestr.Substring(ihtmlte+1);
        } else
        {
            filestr = filestr.Substring(0, ihtmlte + 1) + "<head><title>TEST ENVIRONMENT INFORMATION: " + watermark + "</title></head>" + filestr.Substring(ihtmlte + 1);
        }
