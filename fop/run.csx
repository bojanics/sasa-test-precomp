...
#r "IKVM.OpenJDK.XML.API.dll"
#r "fop-2.0-saxon-9.1.0.8.dll"
#r "itextsharp.dll"
...
public static async Task<HttpResponseMessage> Run(HttpRequestMessage req, TraceWriter log, ExecutionContext context)
{
    ...
    try
    {
        ...
        else
        {
            ...
            //log.Info("XMLAFT=" + xml);
            pdfByteArray = doPDFGen(xml, xsl_transf.value, rootloc, log);
        }
...
private static byte[] doPDFGen(string xml, string xsl, string rootloc, TraceWriter log)
{
    try
    {
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        DefaultConfigurationBuilder cfgBuilder = new DefaultConfigurationBuilder();
        org.apache.avalon.framework.configuration.Configuration cfg = cfgBuilder.buildFromFile(new java.io.File(rootloc + "/userconfig.xml"));
...
{
    ByteArrayOutputStream baos = new ByteArrayOutputStream();
    TransformerFactory tfact = new net.sf.saxon.TransformerFactoryImpl();

    Transformer xsltTrans = tfact.newTransformer(new StreamSource(xsl));

    Source src = new StreamSource(new java.io.StringReader(xml));
    Result res = new StreamResult(baos);

    xsltTrans.setOutputProperty("encoding", "UTF-8");
    xsltTrans.transform(src, res);
    
    byte[] byteArray = baos.toByteArray();

    MemoryStream inFo = new MemoryStream(byteArray);
